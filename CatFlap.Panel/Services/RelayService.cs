...
public sealed class RelayService
{
    ...
    public RelayService(RelayConfigService config, CatFlapRelayManager manager, ILogger<RelayService> logger)
    {
        _config = config;
        _manager = manager;
        _logger = logger;
    ...
    public Task<IReadOnlyList<CatFlapRelayOption>> GetAllOptions() => _config.GetAllAsync();
    ...
    /// </summary>
    public IReadOnlyList<CatFlapRelay> GetAllRelays() => _manager.GetAllRelays();
    ...
    public CatFlapRelayStatistics? GetStatistics(Guid id)
    ...
    /// </summary>
    public async Task AddRelayAsync(CatFlapRelayOption option)
    {
    ...
    /// </summary>
    public async Task UpdateRelayAsync(Guid id, CatFlapRelayOption option)
    {
        ArgumentNullException.ThrowIfNull(option);
        CatFlapRelayFactory.ValidateOption(option);
    ...
    /// </summary>
    public async Task ReplaceAllAsync(IEnumerable<CatFlapRelayOption> options)
    {
        ArgumentNullException.ThrowIfNull(options);
        var list = options.ToList();
        foreach (var o in list)
            CatFlapRelayFactory.ValidateOption(o);
    ...
    public async Task<CatFlapRelayOption?> GetByIdAsync(Guid id)
    {
    ...
    private async Task CheckPortConflictAsync(CatFlapRelayOption option, Guid? excludeId)
    {
        var all = await _config.GetAllAsync();
