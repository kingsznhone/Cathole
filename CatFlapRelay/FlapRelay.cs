...
{
    public class FlapRelay : IAsyncDisposable
    {
        private readonly FlapRelayOption _option;
        private readonly ILogger<FlapRelay> _logger;
        ...
        private static readonly TimeSpan _disposeTimeout = TimeSpan.FromSeconds(30);

        public FlapRelay(FlapRelayOption option, ILogger<FlapRelay> logger)
        {
        ...
        {
            // Reset LastActivity to avoid race where task scheduling delay exceeds tunnel timeout
            tunnelInfo.LastActivity = DateTime.UtcNow;
